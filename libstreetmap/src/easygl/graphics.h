...
bool drawtext(float xc, float yc, const std::string& text, float boundx=FLT_MAX, float boundy=FLT_MAX);
bool drawtext(const t_point& text_center, const std::string& text, float boundx=FLT_MAX, float boundy=FLT_MAX);
 ...
 * that is curreninit_postscripttly visible on the screen is below area_threshold.
 */
...
void search_zoom_in(float per_zoom, float x, float y, void(*drawscreen)(void));
