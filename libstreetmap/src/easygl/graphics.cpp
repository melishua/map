...
bool drawtext_in(const t_bound_box& bbox, const std::string& text) {
    return drawtext(bbox.get_center(), text, bbox);
}

bool drawtext_in(const t_bound_box& bbox, const std::string& text, float tolerance) {
    return drawtext(bbox.get_center(), text, bbox, tolerance);
}
...
bool drawtext(
    ...
    return drawtext(text_center, text, tolerance_bounds);
}

bool drawtext(const t_point& text_center, const std::string& text, const t_bound_box& bounds) {
    ...
    min_bounds *= 2;
    return drawtext(text_center, text, min_bounds.x, min_bounds.y);
}

bool drawtext(const t_point& text_center, const std::string& text, float boundx, float boundy) {
    return drawtext(text_center.x, text_center.y, text, boundx, boundy);
}
 ...
 */
bool drawtext(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
...
handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    ...
    float temp_xleft = x - (x - trans_coord.xleft) / ZOOM_FACTOR;
    float temp_xright = x + (trans_coord.xright - x) / ZOOM_FACTOR;
    //make ybot - ytop = 0.6 of the original distance
    float temp_ytop = y - (y - trans_coord.ytop) / ZOOM_FACTOR;
    float temp_ybot = y + (trans_coord.ybot - y) / ZOOM_FACTOR;
    // test if zoom in will cause function percentage zoom exceed 0.00257125
    float percentage_zoom = (temp_xright - temp_xleft) / (trans_coord.init_xright - trans_coord.init_xleft);
    if (percentage_zoom < 0.0024) {
        return;
    } else {
        trans_coord.xleft = temp_xleft;
        trans_coord.xright = temp_xright;
        trans_coord.ytop = temp_ytop;
        trans_coord.ybot = temp_ybot;
    }
...
handle_zoom_out(float x, float y, void (*drawscreen) (void)) {
    ...
    float temp_ytop = y - (y - trans_coord.ytop) * ZOOM_FACTOR;
    float temp_ybot = y + (trans_coord.ybot - y) * ZOOM_FACTOR;
    // return if zoom will cause the screen be bigger than initial coordinate
    if (temp_xleft < trans_coord.init_xleft || temp_xright > trans_coord.init_xright ||
            temp_ytop > trans_coord.init_ytop || temp_ybot < trans_coord.init_ybot) {
...
translate_up(void (*drawscreen) (void)) {
    ...
    // check if translate will cause screen go out of bound
    float temp_ytop = trans_coord.ytop - ystep;
    float temp_ybot = trans_coord.ybot - ystep;
    if (temp_ytop > trans_coord.init_ytop) {
        float ystep_limit = trans_coord.init_ytop - trans_coord.ytop;
        trans_coord.ytop = trans_coord.init_ytop;
        trans_coord.ybot -= ystep_limit;
    } else {
        // translate down
        trans_coord.ytop = temp_ytop;
        trans_coord.ybot = temp_ybot;
...
translate_down(void (*drawscreen) (void)) {
    ...
    float temp_ytop = trans_coord.ytop + ystep;
    float temp_ybot = trans_coord.ybot + ystep;
    if (temp_ybot < trans_coord.init_ybot) {
        float ystep_limit = trans_coord.init_ybot - trans_coord.ybot;
        trans_coord.ybot = trans_coord.init_ybot;
        trans_coord.ytop += ystep_limit;
    } else {
        // translate up
        trans_coord.ytop = temp_ytop;
        trans_coord.ybot = temp_ybot;
...
translate_left(void (*drawscreen) (void)) {
    ...
    // check if translate will cause screen go out of bound
    float temp_xleft = trans_coord.xleft - xstep;
    float temp_xright = trans_coord.xright - xstep;
    if (temp_xleft < trans_coord.init_xleft) {
        float xstep_limit = trans_coord.init_xleft - trans_coord.xleft;
        trans_coord.xleft = trans_coord.init_xleft;
        trans_coord.xright -= xstep_limit;
    } else {
        trans_coord.xleft = temp_xleft;
        trans_coord.xright = temp_xright;
    }
...
translate_right(void (*drawscreen) (void)) {
    ...
    float temp_xleft = trans_coord.xleft + xstep;
    float temp_xright = trans_coord.xright + xstep;
    if (temp_xright > trans_coord.init_xright) {
        float xstep_limit = trans_coord.init_xright - trans_coord.xright;
        trans_coord.xright = trans_coord.init_xright;
        trans_coord.xleft += xstep_limit;
    } else {
        trans_coord.xleft = temp_xleft;
        trans_coord.xright = temp_xright;
    }
...
panning_execute(int x, int y, void (*drawscreen) (void)) {
    ...
    // check if panning will cause screen go out of bound
    float temp_xleft = trans_coord.xleft - x_change_world;
    float temp_xright = trans_coord.xright - x_change_world;
    float temp_ybot = trans_coord.ybot - y_change_world;
    float temp_ytop = trans_coord.ytop - y_change_world;
    // translate according to panning if in bound, else set to bound
    if (temp_xleft < trans_coord.init_xleft) trans_coord.xleft = trans_coord.init_xleft;
    else trans_coord.xleft = temp_xleft;
    if (temp_xright > trans_coord.init_xright) trans_coord.xright = trans_coord.init_xright;
    else trans_coord.xright = temp_xright;
    if (temp_ybot < trans_coord.init_ybot) trans_coord.ybot = trans_coord.init_ybot;
    else trans_coord.ybot = temp_ybot;
    if (temp_ytop > trans_coord.init_ytop) trans_coord.ytop = trans_coord.init_ytop;
    else trans_coord.ytop = temp_ytop;
...
// pass in latlon as x center and y center

void search_zoom_in(float zoom_factor, float x, float y, void(*drawscreen)(void)) {
    set_coordinate_system(GL_WORLD);
    //make xright - xleft = 0.6 of the original distance 
    trans_coord.xleft = x - (x - trans_coord.xleft) / (1 / zoom_factor);
    trans_coord.xright = x + (trans_coord.xright - x) / (1 / zoom_factor);
    //make ybot - ytop = 0.6 of the original distance
    trans_coord.ytop = y - (y - trans_coord.ytop) / (1 / zoom_factor);
    trans_coord.ybot = y + (trans_coord.ybot - y) / (1 / zoom_factor);
...
int non_static_yworld_to_scrn(float worldy) {
    float winy;
    ...
    int casted_winy = static_cast<int> (winy);

    return (casted_winy);
}

int non_static_xworld_to_scrn(float worldx) {
    float winx;
    ...
    int casted_winx = static_cast<int> (winx);

    return (casted_winx);
}

float non_static_xscrn_to_world(int scrnx) {
    float world_coord_x;
    world_coord_x = scrnx * trans_coord.stow_xmult + trans_coord.xleft;

    return world_coord_x;
}

float non_static_yscrn_to_world(int scrny) {
    float world_coord_y;
    world_coord_y = scrny * trans_coord.stow_ymult + trans_coord.ytop;

    return world_coord_y;
}

void set_screen_bound(double top, double bot, double left, double right, double yc, double xc, void (*drawscreen) (void)) {
    // check input
    double y_len = top - bot;
    double x_len = right - left;
    if (x_len < 0 || y_len < 0) {
        std::cout << "screen bound incorrect" << std::endl;
    }
    // ratio
    double cur_x_len = trans_coord.xright - trans_coord.xleft;
    double cur_y_len = trans_coord.ytop - trans_coord.ybot;
    double y_ratio = y_len / cur_y_len;
    double x_ratio = x_len / cur_x_len;
    float temp_xleft;
    float temp_xright;
    float temp_ytop;
    float temp_ybot;
    // determine new screen bound
    if (x_ratio > y_ratio) { // adjust according to x
        temp_xleft = left;
        temp_xright = right;
        float expected_y_len =  cur_y_len * x_ratio;
        temp_ytop = yc + expected_y_len / 2;
        temp_ybot = yc - expected_y_len / 2;
    } else { // adjust according to y
        temp_ytop = top;
        temp_ybot = bot;
        float expected_x_len = cur_x_len * y_ratio;
        temp_xleft = xc - expected_x_len / 2;
        temp_xright = xc + expected_x_len / 2;
        
    }     
    // adjust
    if (left < trans_coord.init_xleft) trans_coord.xleft = trans_coord.init_xleft;
    else trans_coord.xleft = temp_xleft ;
    if (right > trans_coord.init_xright) trans_coord.xright = trans_coord.init_xright;
    else trans_coord.xright = temp_xright;
    if (bot < trans_coord.init_ybot) trans_coord.ybot = trans_coord.init_ybot;
    else trans_coord.ybot = temp_ybot;
    if (top > trans_coord.init_ytop) trans_coord.ytop = trans_coord.init_ytop;
    else trans_coord.ytop = temp_ytop;
...
void set_screen_bound(double top, double bot, double left, double right, double yc, double xc, void (*drawscreen) (void)) {
    ...
    double y_ratio = y_len / cur_y_len;
    double x_ratio = x_len / cur_x_len;
    float temp_xleft = x_cen - (x_cen - trans_coord.xleft) * factor;
    float temp_xright = x_cen + (trans_coord.xright - x_cen) * factor;
    float temp_ytop = y_cen - (y_cen - trans_coord.ytop) * factor;
    float temp_ybot = y_cen + (trans_coord.ybot - y_cen) * factor;
    
    if (x_ratio > y_ratio) { // adjust according to x
        temp_xleft = left;
        temp_xright = right;
        float expected_y_len =  cur_y_len * x_ratio;
        temp_ytop = y_cen + expected_y_len / 2;
        temp_ybot = y_cen - expected_y_len / 2;
    } else { // adjust according to y
        
        
    }
    double factor = std::max(x_ratio, y_ratio);
    // adjust
    float temp_xleft = x_cen - (x_cen - trans_coord.xleft) * factor;
    float temp_xright = x_cen + (trans_coord.xright - x_cen) * factor;
    float temp_ytop = y_cen - (y_cen - trans_coord.ytop) * factor;
    ...
    std::cout << std::endl << "gtop: " << trans_coord.ytop << " gbot: " << trans_coord.ybot <<  " gright: " << trans_coord.xright << " gleft: " << trans_coord.xleft << std::endl;
    update_transform();
    drawscreen();
}
 * 
